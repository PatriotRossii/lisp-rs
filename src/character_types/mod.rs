...
impl Character for char {
    fn syntax_type(&self) -> SyntaxType {
        match self {
            '\u{8}' => SyntaxType::Constituent {
                r#traits: ConsTrait::Invalid,
            },
            '!' | '$' | '%' | '&' | '*' | '+' | '-' | '<' | '=' | '>' | '?' | '@' | '[' | ']'
            | '^' | '_' | '{' | '}' | '~' => SyntaxType::Constituent {
                r#traits: ConsTrait::Alphabetic { ext: None },
            },
            '.' => SyntaxType::Constituent {
                r#traits: ConsTrait::Alphabetic {
                    ext: Some(AlphabeticExt::Dot {
                        ext: Some(DotExt::DecimalPoint),
                    }),
                },
            },
            '/' => SyntaxType::Constituent {
                r#traits: ConsTrait::Alphabetic {
                    ext: Some(AlphabeticExt::RatioMarker),
                },
            },
            '0'..='9'
            | 'a'..='c'
            ...
            | 'M'..='R'
            | 't'..='z'
            | 'T'..='Z' => SyntaxType::Constituent {
                r#traits: ConsTrait::Alphadigit { ext: None },
            },
            'd'..='f' | 'D'..='F' | 'l' | 's' | 'L' | 'S' => SyntaxType::Constituent {
                r#traits: ConsTrait::Alphadigit {
                    ext: Some(AlphadigitExt::ExponentMarker),
                },
            },
            ':' => SyntaxType::Constituent {
                r#traits: ConsTrait::PackageMarker,
            },
